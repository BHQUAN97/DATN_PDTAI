...
// get list nhạc home
        ...
        const output = this.songs.map((song, index) => {
            ...
            return `
                ...
                index="${index}" id-song="${song.encodeId}">
                    ...
                    <span class="item__song__name">${song.title}</span>
    ...
    renderItemLike(){
        const item = this.listRenderLike.map((item, index) => 
        `
        ...
        document.querySelector('.list__song').innerHTML = item.join('')

    },

    deleteSongFavorite(){
        let itemFavorite = this.currentSong.encodeId
       this.listFavailable.find((id, index) => {
            if(id === itemFavorite){
    ...
    loadSong( play = false,id = this.songs[this.currentIndex].encodeId){
        ...
        Promise.all([info, linkMp3])
            ...
            .then(() =>{
                ...
                const heart = `<i class="ri-heart-fill ${ issFavorite && 'red'}">`
        ...
        // Điều kiện Play khi load </i> <i class="ri-arrow-down-line">
